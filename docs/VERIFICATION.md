...
| JVM testovi | uključeni u `test` | 37/37 prošlo kroz app/core module, uključujući redoslijed poslužiteljskog brisanja računa i dinamički Open Food Facts User-Agent |
| Android instrumentacija | `:core:data:connectedDebugAndroidTest :app:connectedDebugAndroidTest` | 60/60 prošlo lokalno na API 35 te zasebno na API 29 i API 35 u obveznom release runu `29775884269`: 32 Room/repository i 28 app/Compose testova po API-ju |
| Cloud Functions | `npm --prefix functions run build` | TypeScript kompilacija uspješna |
| Firebase Emulator Suite | `npm --prefix functions run test:emulator` | 61/61 prošlo: Firestore/Storage pravila, kanonski nazivi, ID veze, atomske skupne operacije, istodobno offline dodavanje, brisanje računa/prijenos vlasništva i release-workflow sigurnosni testovi |
...
| Workflow sintaksa | parsiranje svih datoteka u `.github/workflows` Node YAML parserom | `android-instrumentation.yml`, `ci.yml`, `deploy-production-backend.yml`, `pages.yml` i `release.yml` valjani; sve vanjske Actions reference pinane na puni commit SHA; release posao ovisi o PASS rezultatu API 29/35 matrice |
| Produkcijski backend | puni deploy Functions/rules/indexes/storage + produkcijski smoke | PASS; 16/16 funkcija ACTIVE iz commita `26378b0`, backend API 7 + `account-deletion:v1`, capability odgovor HTTP 200, 12/12 zaštićenih callable funkcija HTTP 401 bez vjerodajnice |
| Potpisani GitHub Release | Actions run `29775884269` + anonimni ponovni download | PASS; RC28 je prije ručnog `production` odobrenja blokiran do PASS-a API 29 i API 35 instrumentacije; javni APK i manifest potvrđeni HTTP 200 |
| APK manifest | `aapt dump badging` | debug `hr.smocnica.debug`, release `hr.smocnica`, `minSdk 29`, stabilni `targetSdk 36`, `versionCode 28`, `versionName 1.0.0-rc28` |
...
- javni release: `smocnica-1.0.0-rc28.apk`, 30.259.309 bajta, SHA-256 `F90F16D20E958221352393E37AB7387D42B320078F43240372D902CA2240A5CC`;
...
Release APK iz GitHub Releasea potpisan je trajnim produkcijskim ključem iz zaštićenog Environment Secreta; nakon anonimnog ponovnog preuzimanja `apksigner verify --verbose --print-certs` potvrđuje isti očekivani certifikat. Manifestov SHA-256 jednak je stvarnom hashu preuzetog APK-a. RC28 workflow dokazano uvjetuje signing uspješnom API 29/35 instrumentacijom te briše dekodirani keystore i produkcijski `google-services.json` prije generiranja manifesta i objave. Nadogradnja prethodne instalacije i očuvanje podataka i dalje zahtijevaju stvarni uređaj.
...
- GitHub rate-limit ponašanje i obvezna Android nadogradnja rc9→rc19 s očuvanjem lokalnih podataka na dva stvarna uređaja; objava i anonimni javni download aktualnog RC28 potvrđeni su;
...
| produkcijski Firebase deploy | 20. srpnja 2026. iz commita `26378b0`: Firestore/Storage pravila i indeksi objavljeni; svih 16 Node.js 22 funkcija aktivno u `europe-west1`, isti source hash; API 7 post-deploy smoke PASS |
...
Objavljen je potpisani javni RC28 s očekivanim certifikatom, produkcijskom Firebase konfiguracijom i provjerenim update manifestom. Release je prije ručnog odobrenja prošao API 29 i API 35 instrumentacijski gate te zatim zaštićeni `production` Environment; pet signing/Firebase tajni postoje samo kao Environment Secrets, a repozitorijski Secrets istih naziva uklonjeni su. Instalacija, stvarni App Check promet, brisanje testnog računa na stvarnom uređaju i nadogradnja na uređaju moraju se evidentirati u `docs/REAL_DEVICE_TEST_PLAN.md`.
