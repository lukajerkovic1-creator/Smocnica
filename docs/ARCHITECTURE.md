...
- `getBackendCapabilities` je javni, neosjetljivi handshake koji vraća samo `backendApiVersion`, statičke capability oznake i očekivani manifest funkcija. Android prije registracije uređaja i obnove cloud podataka zahtijeva API 5 te `operation:delete_shopping`, `device-registration:v2`, `notification-privacy:v1`, `single-active-pantry:v1`, `canonical-names:v1` i `manual-shopping-merge:v1`. Potvrđena verzija lokalno se pamti samo za privremeni offline fallback; izričito zastario ili nepotpun odgovor uvijek blokira udaljene pozive.
